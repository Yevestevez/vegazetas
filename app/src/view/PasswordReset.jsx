...
function PasswordReset({ onPasswordReseted }) {
    ...
    const token = searchParams.get('token') // ✅ obtiene token de query param
    ...
    const handleFormSubmit = event => {
        ...
        confirm('¿Quieres cambiar tu contraseña?', accepted => {
            ...
            const loadingAlertTimeout = setTimeout(() => {
                closeAlert = alert(
                    'El servidor está despertando... Solo espera mientras cambiamos tu contraseña',
                    { isWakingServer: true }
                )
            }, 2000)
            ...
            try {
                logic.passwordReset(token, newPassword)
                    ...
                    .catch(error => {
                        ...
                        else if (error instanceof SystemError) alert('Sorry try again')
                    })
            } catch (error) {
                clearTimeout(loadingAlertTimeout)
                if (closeAlert) closeAlert()
    ...
    return (
        <div className="flex flex-col min-h-screen min-w-screen items-center justify-center gap-12 xl:gap-12 bg-folly">
            <header>
            ...
            <main className="flex flex-col items-center justify-center w-full max-w-7xl gap-6 xs:gap-8 px-8 xs:px-20 pb-10 xs:pb-20">
                <h1 className="text-center anybody-title text-spring-bud text-2xl lg:text-4xl xl:text-5xl leading-tight lg:leading-tight xl:leading-tight">
                    Restablecimiento de contraseña
                </h1>

                <form onSubmit={handleFormSubmit} className="flex flex-col items-center justify-center w-full gap-4">
                    <label htmlFor="password" className="anybody text-xl lg:text-2xl font-bold text-spring-bud">
                        Nueva contraseña
                    </label>
                    <Input
                        type="password"
                    ...
                    <CircleButton
                        type="submit"
                        className="bg-spring-bud text-folly hover:outline hover:outline-spring-bud hover:bg-folly hover:text-spring-bud mt-4"
                    >
                        Guardar
                    </CircleButton>
                </form>
