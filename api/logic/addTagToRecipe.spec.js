        ...
        return User.create({ name: 'Ron Weasley', email: 'ron@weasley.com', username: 'ronweasley', password: '123123123' })
            .then(user => {
                return Recipe.create({
                    ...
                    .then(recipe => {
                        return addTagToRecipe(user._id.toString(), recipe._id.toString(), 'Arroz')
                    })
                    .then((result) => {
                    ...
                    .then(recipe => {
                        expect(recipe.tags).to.be.an('array').with.lengthOf(5)
                        expect(recipe.tags[4]).to.equal('Arroz')
                    })
            })
    })
        ...
        return User.create({ name: 'Ron Weasley', email: 'ron@weasley.com', username: 'ronweasley', password: '123123123' })
            .then(user => {
                return Recipe.create({
                    ...
                    .then(recipe => {
                        return addTagToRecipe(new ObjectId().toString(), recipe._id.toString(), 'Arroz')
                    })
        ...
        return User.create({ name: 'Ron Weasley', email: 'ron@weasley.com', username: 'ronweasley', password: '123123123' })
            .then(user => {
                return Recipe.create({
                    ...
                    .then(recipe => {
                        return addTagToRecipe(user._id.toString(), new ObjectId().toString(), 'Arroz')
                    })
                    .catch(error => catchedError = error)
                    .finally(() => {
                        expect(catchedError).to.be.instanceOf(NotFoundError)
